...
namespace Nightmare
{
    ...
    {
        [Tooltip("Layers de grounded")]
        public LayerMask groundedLayerMask;
        [Tooltip("Distancia de raycast para grounded")]
        ...
        public void Teleport(Vector2 destinationPosition)
        {
            Vector2 delta = destinationPosition - m_CurrentPosition;
            m_PreviousPosition += delta;
            m_CurrentPosition = destinationPosition;
            m_Rigidbody2D.MovePosition(destinationPosition);
        }

        /// <summary>
        /// atualiza valor de IsGrounded. call feita no fixedUpdate, mas pode ser chamado mais frequentemente para garantir melhor precisao
        /// </summary>
        /// <param name="bottom">not required</param>
        public void CheckCapsuleEndCollisions(bool bottom = true)
        {
            ...
            else
            {
                ...
                {
                    raycastDirection = Vector2.down;
                    Vector2 raycastStartBottomCenter = raycastStart + Vector2.down * (m_Capsule.size.y * 0.5f - m_Capsule.size.x * 0.5f);

                    m_RaycastPositions[0] = raycastStartBottomCenter + Vector2.left * m_Capsule.size.x * 0.5f;
                    m_RaycastPositions[1] = raycastStartBottomCenter;
                    m_RaycastPositions[2] = raycastStartBottomCenter + Vector2.right * m_Capsule.size.x * 0.5f;
                }
                else
                {
                    raycastDirection = Vector2.up;
                    Vector2 raycastStartTopCenter = raycastStart + Vector2.up * (m_Capsule.size.y * 0.5f - m_Capsule.size.x * 0.5f);

                    m_RaycastPositions[0] = raycastStartTopCenter + Vector2.left * m_Capsule.size.x * 0.5f;
                    m_RaycastPositions[1] = raycastStartTopCenter;
                    m_RaycastPositions[2] = raycastStartTopCenter + Vector2.right * m_Capsule.size.x * 0.5f;
                }
            }
